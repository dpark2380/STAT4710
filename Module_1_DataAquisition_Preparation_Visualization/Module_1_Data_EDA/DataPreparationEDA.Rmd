...
[DPLYR Cheat Sheet](http://github.com/rstudio/cheatsheets/blob/main/data-transformation.pdf)
...
```{r }
# First selecting rows, next columns.
...
baseball %>% group_by(team) %>% mutate(max_win = max(win_pct)) %>% arrange(-max_win, -win_pct) ### Is this a good idea? 
```
...
```{r, facet hist}
p1 <- ggplot(data_agg) + 
  geom_histogram(aes(x = payroll_total), bins = 20, fill = "blue") +
...
# Regression minimises vertical distance. We can only chnage payroll amount and analyse the winning percentage.
# We can't change the winning percentage then try to evaluate the correct payroll amount. We need a regression line with the
# variables switched.

data_agg %>%
  ggplot(aes(x = win_pct_ave, y = payroll_total)) + 
  geom_point(aes(color = team), size = 3) + 
