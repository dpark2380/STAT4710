...
In the following analyses, we focus on the effects of `beds`, `fbath`, `bldg`, `lake_michigan_dist_ft` and `neighborhood` on `log_price`. 
Run a regression of `log_price` vs. `beds`, `fbath`, `bldg`, `lake_michigan_dist_ft` and  `neighborhood` without interactions as `fit1`. Report the summary of `fit1`. Answering the following questions based on `fit1`. 

## Question 2-1.

**i.** In this model is `lake_michigan_dist_ft` a useful variable at .01 level?  

**ii.**   Use no more than three sentences to interpret the `lake_michigan_dist_ft` effect on the `log_price`.
...
**iv.** Gather variable names selected by LASSO and `neighborhoo` as lasso_selected <- c("neighborhood",... )
