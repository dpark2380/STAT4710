...
## Grouping the ages into separate bins.
survey <- survey %>%
...
# Selecting order in which to display income
income_order <- c(
  "NA",
  "Less than $15,000",
  "$15,000 - $30,000",
...
# Plotting listening rate as income increases for both platforms
survey %>%
...
### Does this sample appear to be a random sample from the general population of the USA? Why it is crucial to have randomness here?

```{r}
total <- nrow(survey)

# Showing counts and proportions of income and gender.
survey %>% count(income) %>% mutate(percent = 100 * n / sum(n))
survey %>% count(gender) %>% mutate(percent = 100 * n / sum(n))

# Binning ages again, finding counts and proportions
survey <- survey %>%
  mutate(
    age_num = suppressWarnings(as.numeric(trimws(age)))
  )
...
### Proposal: Estimating the Current Audience Size of Wharton Business Radio (Budget: $1000, Timeline: 2 months)

#### 1) Method
We will estimate the number of current listeners as follows:

1. Collect U.S. adult samples to identify individuals who are SiriusXM listeners

2. Within the SiriusXM listener subset, estimate the proportion who have listened to Wharton Business Radio in the last 30 days.
Let p be the estimated proportion of Wharton listeners among SiriusXM listeners. We will multiply p by a credible estimate of the **current SiriusXM listener/subscriber base** from public sources to obtain an estimated audience size. A 95% confidence interval will be computed using a standard CI for a proportion and scaled to the population size.

#### 2) Data to collect and sources
**Survey variables**

- SiriusXM listening status (Yes/No, past 30 days)

- Wharton Business Radio listening status (Yes/No, past 30 days)

- Listening frequency (approximate minutes per week, past 30 days)

- Medium of Listening (car radio, SiriusXM app, podcast replay, other)

- Demographics for weighting: age, gender, education, household income, region/state

**Collection of Data**

We will use:

- **Prolific** platform to collect data, a platform which "helps AI developers, researchers, and organizations easily access the highest-quality human data" (Prolific)

- Publicly available estimates of **current SiriusXM subscriber/listener base** (For example, company reports/filings or reputable summaries).

**Budgeting**
The $1000 budget will be distributed as follows:

- Participant incentives (~$850):
  Used to recruit approximately 700–900 U.S.-based respondents on Prolific for a 5-7 minute survey. Incentive rates are set to meet platform minimums while ensuring response quality is sufficient.

- Platform and service fees (~$100):  
  Covers Prolific service fees and any additional costs associated with the collection of data.

- Contingency and pilot testing (~$50):
  Reserved for a small pilot survey to test question clarity and for additional responses if data quality issues arise.

**Pros and Cons**

Pros

- Access to a high-quality, pre-screened participant pool with reliable demographic information

- Better data quality and fewer bots or low-effort responses compared to MTurk

- Ability to target U.S. respondents and apply demographic quotas, reducing sampling bias

- Cost-effective for small to medium-sized academic studies within a limited budget

Cons:

- More expensive per response than MTurk, limiting maximum sample size

- Still not fully representative of the general U.S. population, requiring weighting and careful interpretation


**Use of ChatGPT**

ChatGPT can assist with survey wording, analysis planning, and drafting of the reports, but it will not be used as an empirical source for audience size, as the data sources that it pulls cannot easily be verified, and ChatGPT can also hallucinate, providing incorrect modelling data. 

**Final Deliverable**

Point estimate + 95% CI for current audience size, as well as outlining the limitations of the data collected in relation to several factors, such as potential selection bias arising from the survey platform, and uncertainty in extrapolating survey proportions to the broader listening population. 
