...
**ASVAB** ([Armed Services Vocational Aptitude Battery](https://www.officialasvab.com/)) tests have been used as a screening test for those who want to join the army or other jobs. It helps to determine which army jobs are appropriate for applicants. No preparation is needed. 
...
```{r, data.full.skim}
data.full <- read.csv("data/IQ.Full.csv")  
...
# Taking 50 samples from the dataset. 
...
```{r scatter plot of original scaled data}
# Extracting only pargaraph and word data. This will be 50 observations of 2 variables.
...
p <- ggplot(data = parag.word.scaled.centered, 
  ...
  geom_text(data = loadings,
            ...
            col = "red") +
  theme_bw() +
  coord_fixed(ratio = 1, xlim = c(-2, 2.5), ylim = c(-2, 2.5)) +
...
```{r pca_biplot_clean, message=FALSE, warning=FALSE}
library(factoextra)
